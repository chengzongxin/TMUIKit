...
@property(nonatomic, assign, readwrite) TMUITableViewCellPosition cellPosition;
...
@property(nonatomic, strong) TMUIButton *defaultAccessoryButton;
@property(nonatomic, strong) UIView *defaultDetailDisclosureView;
@end

@implementation TMUITableViewCell
...
- (instancetype)initForTableView:(UITableView *)tableView withStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self = [self initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [self didInitializeWithStyle:style];// 因为设置了 parentTableView，样式可能都需要变，所以这里重新执行一次 didInitializeWithStyle: 里的 tmui_styledAsTMUITableViewCell
    }
    return self;
}
...
// TMUITableViewCell 由于 init 时就把 tableView 传进来，所以可以在更早的时机拿到 tmui_tableView 的值，如果是系统的 UITableView，默认只能在添加到 tableView 上之后才可以获取到引用
- (UITableView *)tmui_tableView {
    return self.parentTableView ?: [super tmui_tableView];
}
...
- (void)setEnabled:(BOOL)enabled {
    if (_enabled != enabled) {
        if (enabled) {
            ...
            UIColor *titleLabelColor = self.tmui_styledTextLabelColor;
            if (titleLabelColor) {
...
- (void)initDefaultAccessoryButtonIfNeeded {
    if (!self.defaultAccessoryButton) {
        self.defaultAccessoryButton = [[TMUIButton alloc] init];
...
// 重写accessoryType，如果是UITableViewCellAccessoryDisclosureIndicator类型的，则使用 TMUIConfigurationTemplate.m 配置表里的图片
...
- (void)handleAccessoryButtonEvent:(TMUIButton *)detailButton {
    if ([self.tmui_tableView.delegate respondsToSelector:@selector(tableView:accessoryButtonTappedForRowWithIndexPath:)]) {
        [self.tmui_tableView.delegate tableView:self.tmui_tableView accessoryButtonTappedForRowWithIndexPath:[self.tmui_tableView tmui_indexPathForRowAtView:detailButton]];
    }
}

@end

@implementation TMUITableViewCell(TMUISubclassingHooks)

- (void)didInitializeWithStyle:(UITableViewCellStyle)style {
    self.initByTableView = NO;
    _cellPosition = TMUITableViewCellPositionNone;
    ...
    [self tmui_styledAsTMUITableViewCell];
}

- (void)updateCellAppearanceWithIndexPath:(NSIndexPath *)indexPath {
    // 子类继承
    if (indexPath && self.tmui_tableView) {
        TMUITableViewCellPosition position = [self.tmui_tableView tmui_positionForRowAtIndexPath:indexPath];
        self.cellPosition = position;
    } else {
        self.cellPosition = TMUITableViewCellPositionNone;
    }
}
