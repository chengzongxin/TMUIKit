...
@interface TMUITableViewCell : UITableViewCell
 ...
 *
 *  @note UITableViewCell(TMUI) 内也有一个 qmui_cellPosition，那个需要在 willDisplayCell 后才有值，cellForRow 里是用不了的，所以 TMUITableViewCell 才增加 cellPosition。
 */
@property(nonatomic, assign, readonly) TMUITableViewCellPosition cellPosition;
...
@interface TMUITableViewCell (TMUISubclassingHooks)
