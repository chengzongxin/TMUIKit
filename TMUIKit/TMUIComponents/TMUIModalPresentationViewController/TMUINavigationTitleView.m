...
@implementation TMUINavigationTitleView

#pragma mark - 初始化

- (instancetype)initWithFrame:(CGRect)frame {
    return [self initWithStyle:TMUINavigationTitleViewStyleDefault frame:frame];
}
...
- (instancetype)initWithStyle:(TMUINavigationTitleViewStyle)style frame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.accessoryType = TMUINavigationTitleViewAccessoryTypeNone;
        
        [self tmui_applyAppearance];
        self.horizontalTitleFont = TMUINavigationTitleView.appearance.horizontalTitleFont ?: UINavigationBar.tmui_appearanceConfigured.titleTextAttributes[NSFontAttributeName];
        self.horizontalSubtitleFont = TMUINavigationTitleView.appearance.horizontalSubtitleFont ?: self.horizontalTitleFont;
        
        self.adjustsSubviewsTintColorAutomatically = YES;
        self.tintColor = TMUICMIActivated ? NavBarTitleColor : UINavigationBar.tmui_appearanceConfigured.titleTextAttributes[NSForegroundColorAttributeName];
    }
    return self;
}
...
- (CGSize)contentSize {
    ...
    if (self.style == TMUINavigationTitleViewStyleSubTitleVertical) {
...
- (void)setHorizontalTitleFont:(UIFont *)horizontalTitleFont {
    ...
    if (self.style == TMUINavigationTitleViewStyleDefault) {
...
- (void)setAccessoryType:(TMUINavigationTitleViewAccessoryType)accessoryType {
    
    // 如果已设置了accessoryView，则accessoryType不生效
    if (self.accessoryView) {
        accessoryType = TMUINavigationTitleViewAccessoryTypeNone;
    }
    
    _accessoryType = accessoryType;
    
    if (accessoryType == TMUINavigationTitleViewAccessoryTypeNone) {
    ...
    if (accessoryType == TMUINavigationTitleViewAccessoryTypeDisclosureIndicator) {
        accessoryImage = [NavBarAccessoryViewTypeDisclosureIndicatorImage tmui_imageWithOrientation:UIImageOrientationUp];
    }
...
- (void)updateSubAccessoryViewHidden {
    if (self.subAccessoryView && self.subtitleLabel.text.length && self.style == TMUINavigationTitleViewStyleSubTitleVertical) {
...
- (void)setActive:(BOOL)active {
    ...
    if (self.accessoryType == TMUINavigationTitleViewAccessoryTypeDisclosureIndicator) {
        if (active) {
            [UIView animateWithDuration:.25f delay:0 options:TMUIViewAnimationOptionsCurveIn animations:^(void){
...
- (void)setStyle:(TMUINavigationTitleViewStyle)style {
    _style = style;
    if (style == TMUINavigationTitleViewStyleSubTitleVertical) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        OverrideImplementation([UINavigationBar class], @selector(layoutSubviews), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UINavigationBar *selfObject) {
                ...
                TMUINavigationTitleView *titleView = (TMUINavigationTitleView *)selfObject.topItem.titleView;
                
                if ([titleView isKindOfClass:[TMUINavigationTitleView class]]) {
        ...
        // 让 -[UIViewController setTitle:] 可以自动刷新 TMUINavigationTitle
        OverrideImplementation([UIViewController class], @selector(setTitle:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UIViewController *selfObject, NSString *title) {
                ...
                if ([selfObject.navigationItem.titleView isKindOfClass:TMUINavigationTitleView.class]) {
                    ((TMUINavigationTitleView *)selfObject.navigationItem.titleView).title = title;
                }
            };
        });
        
        // 让 -[UINavigationItem setTitle:] 可以自动刷新 TMUINavigationTitleView
        OverrideImplementation([UINavigationItem class], @selector(setTitle:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UINavigationItem *selfObject, NSString *title) {
                ...
                if ([selfObject.titleView isKindOfClass:TMUINavigationTitleView.class]) {
                    ((TMUINavigationTitleView *)selfObject.titleView).title = title;
                }
            };
        });
        ...
        OverrideImplementation([UINavigationItem class], @selector(setTitleView:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UINavigationItem *selfObject, TMUINavigationTitleView *titleView) {
                ...
                if ([titleView isKindOfClass:TMUINavigationTitleView.class]) {
                    if (titleView.title.length <= 0) {
                        NSString *title = selfObject.tmui_viewController.title ?: selfObject.title;
...
@interface TMUINavigationTitleView (UIAppearance)

@end

@implementation TMUINavigationTitleView (UIAppearance)
...
+ (void)setDefaultAppearance {
    TMUINavigationTitleView *appearance = [TMUINavigationTitleView appearance];
    ...
    appearance.verticalSubtitleFont = UIFontLight(12);
...
@implementation TMUINavigationTitleView (LargeTitleCompatibility)

- (void)setAlpha:(BOOL)alpha animated:(BOOL)animated {
    // 在 push 和 pop 过渡期间系统会对自定义的 titleView 的 alpha 进行调整，了避免和系统的设置冲突（比如设置 alpha 为 0 又被系统还原为 1）这里通过设置 contentView 的 alpha 来控制整个 TMUINavigationTitleView 显示与隐藏。
    ...
    dispatch_once(&onceToken, ^{
        ...
        OverrideImplementation([UINavigationController class], sel_registerName("_updateTopViewFramesToMatchScrollOffsetInViewController:contentScrollView:topLayoutType:"), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UINavigationController *selfObject, UIViewController *viewController, UIScrollView *scrollView, NSUInteger topLayoutType) {
                ...
                [selfObject tmui_updateTitleViewToMatchScrollOffsetInViewController:viewController contentScrollView:scrollView topLayoutType:topLayoutType];
            };
        });
    });
}

- (void)tmui_updateTitleViewToMatchScrollOffsetInViewController:(UIViewController *)viewController contentScrollView:(UIScrollView *)contentScrollView topLayoutType:(NSInteger)topLayoutType {
    UIView *titleView = viewController.navigationItem.titleView;
    if (!titleView || ![titleView isKindOfClass:[TMUINavigationTitleView class]]) {
        return;
    }
    
    NSAssert(viewController.navigationController == self, @"TMUINavigationTitleView", @"不存在 UINavigationController");
    
    TMUINavigationTitleView *navigationTitleView = (TMUINavigationTitleView *)titleView;
//    UIView *largeTitleView = self.navigationBar.tmui_largeTitleView;
//    BOOL largeTitleLabelVisable = self.navigationBar.prefersLargeTitles && viewController.tmui_prefersLargeTitleDisplayed && largeTitleView.alpha != 0;
    
    BOOL largeTitleLabelVisable = NO;
