...
typedef void(^TMUISegmentControlItemEventBlock)(UIButton *button, NSInteger index);
typedef void (^TMUIIndexBlock)(NSInteger value);

@interface TMUISegmentControl : UIControl

@property (nonatomic, assign) CGFloat contentLeftMargin;///< 内容视图左侧的间距，默认为0 | 9.2.0 add 部分UI微调

/**
 手动点击和外部调用changeValueToIndex:animation:会执行这个block
 */
@property (nonatomic, copy  ) TMUIIndexBlock     blockValueChanged;

/**
 当对当前选中的菜单项再次点击时，可响应此回调
 @note 通常情况下不需要此回调，一些特定场景需要处理相同菜单项重复点击的响应逻辑时可赋值
 */
@property (nonatomic, copy  ) TMUIIndexBlock     repeatClickItemBlock;

/**
 点击事件的回调
 */
@property (nonatomic, copy  ) TMUISegmentControlItemEventBlock     itemClickBlock;

/**
 曝光的回调
 */
@property (nonatomic, copy  ) TMUISegmentControlItemEventBlock     itemExposeBlock;
