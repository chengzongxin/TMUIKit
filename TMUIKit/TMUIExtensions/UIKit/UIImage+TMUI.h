...
typedef NS_ENUM(NSInteger, TMUIImageResizingMode) {
    TMUIImageResizingModeScaleToFill            = 0,    // 将图片缩放到给定的大小，不考虑宽高比例
    TMUIImageResizingModeScaleAspectFit         = 10,   // 默认的缩放方式，将图片保持宽高比例不变的情况下缩放到不超过给定的大小（但缩放后的大小不一定与给定大小相等），不会产生空白也不会产生裁剪
    TMUIImageResizingModeScaleAspectFill        = 20,   // 将图片保持宽高比例不变的情况下缩放到不超过给定的大小（但缩放后的大小不一定与给定大小相等），若有内容超出则会被裁剪。若裁剪则上下居中裁剪。
    TMUIImageResizingModeScaleAspectFillTop,            // 将图片保持宽高比例不变的情况下缩放到不超过给定的大小（但缩放后的大小不一定与给定大小相等），若有内容超出则会被裁剪。若裁剪则水平居中、垂直居上裁剪。
    TMUIImageResizingModeScaleAspectFillBottom          // 将图片保持宽高比例不变的情况下缩放到不超过给定的大小（但缩放后的大小不一定与给定大小相等），若有内容超出则会被裁剪。若裁剪则水平居中、垂直居下裁剪。
};

typedef NS_ENUM(NSInteger, TMUIImageGradientType) {
    TMUIImageGradientTypeHorizontal,
    TMUIImageGradientTypeVertical,
    TMUIImageGradientTypeTopLeftToBottomRight,
    TMUIImageGradientTypeTopRightToBottomLeft,
    TMUIImageGradientTypeRadial,
};
/**扩展UIImage方法*/
 ...
 *  @return 切割后的新图片
 */
- (nullable UIImage *)tmui_imageWithClippedRect:(CGRect)rect;
 ...
 *  @return 切割后的新图片
 */
- (nullable UIImage *)tmui_imageWithClippedCornerRadius:(CGFloat)cornerRadius;

/**
 *  同上，可以设置 scale
 */

- (nullable UIImage *)tmui_imageWithClippedCornerRadius:(CGFloat)cornerRadius scale:(CGFloat)scale;

/**
 *  将原图以 TMUIImageResizingModeScaleAspectFit 的策略缩放，使其缩放后的大小不超过指定的大小，并返回缩放后的图片。缩放后的图片的倍数保持与原图一致。
 *  @param size 在这个约束的 size 内进行缩放后的大小，处理后返回的图片的 size 会根据 resizingMode 不同而不同，但必定不会超过 size。
 *
 *  @return 处理完的图片
 *  @see tmui_imageResizedInLimitedSize:resizingMode:scale:
 */
- (nullable UIImage *)tmui_imageResizedInLimitedSize:(CGSize)size;

/**
 *  将原图按指定的 TMUIImageResizingMode 缩放，使其缩放后的大小不超过指定的大小，并返回缩放后的图片，缩放后的图片的倍数保持与原图一致。
 *  @param size 在这个约束的 size 内进行缩放后的大小，处理后返回的图片的 size 会根据 resizingMode 不同而不同，但必定不会超过 size。
 ...
 *  @see tmui_imageResizedInLimitedSize:resizingMode:scale:
 */
- (nullable UIImage *)tmui_imageResizedInLimitedSize:(CGSize)size resizingMode:(TMUIImageResizingMode)resizingMode;

/**
 *  将原图按指定的 TMUIImageResizingMode 缩放，使其缩放后的大小不超过指定的大小，并返回缩放后的图片。
 *  @param size 在这个约束的 size 内进行缩放后的大小，处理后返回的图片的 size 会根据 resizingMode 不同而不同，但必定不会超过 size。
 *  @param resizingMode 希望使用的缩放模式
...
@interface UIImage (FullWatermark)
