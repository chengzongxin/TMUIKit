...
// 私有的类，专用于实现 TMUITextViewDelegate，避免 self.delegate = self 的写法（以前是 TMUITextView 自己实现了 delegate）
@interface _TMUITextViewDelegator : NSObject <TMUITextViewDelegate>

@property(nonatomic, weak) TMUITextView *textView;
@end

@interface TMUITextView ()

@property(nonatomic, assign) BOOL debug;
@property(nonatomic, assign) BOOL postInitializationMethodCalled;
@property(nonatomic, strong) _TMUITextViewDelegator *delegator;
...
- (void)_tmui_setTextForShouldChange:(NSString *)text {
...
- (void)handleTextChanged:(id)sender {
    TMUITextView *textView = nil;
    if ([sender isKindOfClass:[NSNotification class]]) {
        id object = ((NSNotification *)sender).object;
        if (object == self) {
            textView = (TMUITextView *)object;
        }
    } else if ([sender isKindOfClass:[TMUITextView class]]) {
        textView = (TMUITextView *)sender;
    }
    ...
    // https://github.com/Tencent/TMUI_iOS/issues/1168
    ...
    if (!textView.markedTextRange && [textView lengthWithString:textView.text] > textView.maximumTextLength) {
        NSString *finalText = [textView.text tmui_substringAvoidBreakingUpCharacterSequencesWithRange:NSMakeRange(0, textView.maximumTextLength) lessValue:YES countingNonASCIICharacterAsTwo:textView.shouldCountingNonASCIICharacterAsTwo];
    ...
    if ([textView.delegate respondsToSelector:@selector(textView:newHeightAfterTextChanged:)]) {
        ...
        if (textView.debug) NSLog(NSStringFromClass(textView.class), @"handleTextDidChange, text = %@, resultHeight = %f", textView.text, resultHeight);
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [textView tmui_scrollCaretVisibleAnimated:NO];
    });
}
...
- (void)setFrame:(CGRect)frame {
    ...
    // https://github.com/Tencent/TMUI_iOS/issues/557
...
- (void)setContentOffset:(CGPoint)contentOffset animated:(BOOL)animated {
    if (!self.shouldRejectSystemScroll) {
        ...
        if (self.debug) NSLog(NSStringFromClass(self.class), @"%@, contentOffset.y = %.2f", NSStringFromSelector(_cmd), contentOffset.y);
    } else {
        if (self.debug) NSLog(NSStringFromClass(self.class), @"被屏蔽的 %@, contentOffset.y = %.2f", NSStringFromSelector(_cmd), contentOffset.y);
    }
}
...
@implementation _TMUITextViewDelegator

#pragma mark - <TMUITextViewDelegate>

- (BOOL)textView:(TMUITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if (self.textView.debug) NSLog(NSStringFromClass(self.class), @"textView.text(%@ | %@) = %@\nmarkedTextRange = %@\nrange = %@\ntext = %@", @(textView.text.length), @(textView.text.tmui_lengthWhenCountingNonASCIICharacterAsTwo), textView.text, textView.markedTextRange, NSStringFromRange(range), text);
    ...
    if (textView.maximumTextLength < NSUIntegerMax) {
        ...
        if (NSMaxRange(range) > textView.text.length) {
            ...
            // https://github.com/Tencent/TMUI_iOS/issues/377
            // https://github.com/Tencent/TMUI_iOS/issues/1170
            ...
            if (range.length > 0) {
                UITextRange *textRange = [self.textView tmui_convertUITextRangeFromNSRange:range];
                [self.textView replaceRange:textRange withText:text];
            }
            return NO;
        }
        
        NSUInteger rangeLength = textView.shouldCountingNonASCIICharacterAsTwo ? [textView.text substringWithRange:range].tmui_lengthWhenCountingNonASCIICharacterAsTwo : range.length;
        ...
        if (textWillOutofMaximumTextLength) {
            ...
            if (substringLength > 0 && [textView lengthWithString:text] > substringLength) {
                NSString *allowedText = [text tmui_substringAvoidBreakingUpCharacterSequencesWithRange:NSMakeRange(0, substringLength) lessValue:YES countingNonASCIICharacterAsTwo:textView.shouldCountingNonASCIICharacterAsTwo];
                if ([textView lengthWithString:allowedText] <= substringLength) {
                    [textView _tmui_setTextForShouldChange:[textView.text stringByReplacingCharactersInRange:range withString:allowedText]];
