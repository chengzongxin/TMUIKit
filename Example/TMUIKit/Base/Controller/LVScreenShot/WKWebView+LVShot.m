...
-(void)shotScreenContentScrollCapture:(void(^)(UIImage*screenShotImage))completion{
    ...
    [self shotScreenContentScrollCaptureWithIndex:0 MaxIndex:page completion:completion];
}

- (void)shotScreenContentScrollCaptureWithIndex:(int)index MaxIndex:(int)maxIndex completion:(void (^)(UIImage *))completion {
    //获取父view
    UIView *superview;
    UIViewController *currentViewController = [UIViewController currentVC];
    ...
    //添加遮盖
    UIView *snapShotCoverView = [superview snapshotViewAfterScreenUpdates:YES];
    snapShotCoverView.frame = CGRectMake(superview.frame.origin.x, superview.frame.origin.y, snapShotCoverView.frame.size.width, snapShotCoverView.frame.size.height);
    
    [superview addSubview:snapShotCoverView];
    ...
    CGFloat boundsH = self.scrollView.bounds.size.height;
    CGSize contextSize;
    if ((maxIndex + 1) * boundsH > self.scrollView.contentSize.height) {
        contextSize = self.scrollView.contentSize;
    } else {
        contextSize = CGSizeMake(self.scrollView.contentSize.width, boundsH * (maxIndex + 1));
    }
...
//    UIGraphicsBeginImageContextWithOptions(self.scrollView.contentSize, NO, UIScreen.mainScreen.scale);
    __weak typeof(self) weakself = self;
    [self shotScreenContentScrollPageDrawWithIndex:index MaxIndex:maxIndex DrawCallBack:^{
