...
+ (CUIStaticRow *)rowWithItems:(NSArray *)items{
    if (items.count == 2) {
        return Row.str(items[0]).fnt(15).cellHeightAuto.color(TableViewCellTitleLabelColor).detailColor(TableViewCellDetailLabelColor).onClick(^{
        ...
        return Row.str(items[0]).fnt(18).detailStr(items[1]).subtitleStyle.cellHeightAuto.color(TableViewCellTitleLabelColor).detailColor(TableViewCellDetailLabelColor).onClick(^{
