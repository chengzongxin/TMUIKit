...
const CGSize TMUIFloatLayoutViewAutomaticalMaximumItemSize = {-1, -1};

@implementation TMUIFloatLayoutView
...
- (void)didInitialize {
    ...
    self.maximumItemSize = TMUIFloatLayoutViewAutomaticalMaximumItemSize;
}
...
- (CGSize)layoutSubviewsWithSize:(CGSize)size shouldLayout:(BOOL)shouldLayout {
    ...
    CGSize maximumItemSize = CGSizeEqualToSize(self.maximumItemSize, TMUIFloatLayoutViewAutomaticalMaximumItemSize) ? CGSizeMake(size.width - UIEdgeInsetsGetHorizontalValue(self.padding), size.height - UIEdgeInsetsGetVerticalValue(self.padding)) : self.maximumItemSize;
    NSInteger line = -1;
