...
@interface ChainUIViewController ()

@end

@implementation ChainUIViewController
...
- (void)demo1 {
    ...
    UILabel *quiz = Label.str(@"%d+%d=?", 1, 1).fnt(@17).txtColor(@"66,66,66").fitSize.x(moose.maxX + 10).cy(moose.cy);
    ...
    GroupTV(
            ...
            Section(
                    Row.custom(^(id contentView) {
                        Label.str(@"Done").txtColor(@"orange").centerAlignment.embedIn(contentView);
                    }).onClick(^{
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    }).embedIn(self.view, CUINull, 10, 20, 10).tg(999);
}
