...
- (id<MTFYRichTextDelegate>)delegate {
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setDelegate:(id<MTFYRichTextDelegate>)delegate {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    [self richTextFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        @strongify(self);
        if (self.clickBlock) {
            self.clickBlock (string , range , index);
        }

        if ([self delegate] && [[self delegate] respondsToSelector:@selector(didClickRichText:range:index:)]) {
            [[self delegate] didClickRichText:string range:range index:index];
        }

        if (self.isClickEffect) {
            [self saveEffectDicWithRange:range];
            [self clickEffectWithStatus:YES];
        }
    }];
}
