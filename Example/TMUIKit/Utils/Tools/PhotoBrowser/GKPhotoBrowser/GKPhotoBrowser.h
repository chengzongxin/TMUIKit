...
#pragma mark - Mtatfy 添加
/// 选择PhotoView时回调
@property(nonatomic, copy) void (^didSelectAtIndexBlock)(NSInteger index);
/// 长按PhotoView时回调
@property(nonatomic, copy) void (^longPressWithIndexBlock)(NSInteger index);
/// 消失后回调
...
/**
 ...
 @param vc 控制器
 @param block 选择PhotoView时回调
 */
- (void)showFromVC:(UIViewController *)vc didSelectedIndexBlock:(nullable void (^)(NSInteger index))block;
