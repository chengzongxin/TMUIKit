...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.didSelectAtIndexBlock) {
        self.didSelectAtIndexBlock(self.currentIndex);
    }
...
- (void)showFromVC:(UIViewController *)vc {
    [self showFromVC:vc didSelectedIndexBlock:nil];
}
- (void)showFromVC:(UIViewController *)vc dismissHandle:(void (^)(void))dismissBlock {
    [self showFromVC:vc];
    self.dismissBlock = dismissBlock;
}

- (void)showFromVC:(UIViewController *)vc didSelectedIndexBlock:(void (^)(NSInteger index))block {
    if (block || !self.didSelectAtIndexBlock) {
        self.didSelectAtIndexBlock = block;
    }
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)longPress {
    switch (longPress.state) {
        case UIGestureRecognizerStateBegan:{
            ...
            if (self.longPressWithIndexBlock) {
                self.longPressWithIndexBlock(self.currentIndex);
            }
        }
            break;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (self.didSelectAtIndexBlock) {
        self.didSelectAtIndexBlock(index);
    }
