...
//        NSString *title = [self decimalwithFormat:@"0" floatV:value.floatValue];
        NSString *title = [NSString stringWithFormat:@"%g",value.floatValue];
        if (value.floatValue < 0) {
            value = @(0);
        }
        
        // 最大
        NSNumber *max =  self.xValues.lastObject;
        NSInteger maxPrice = max.integerValue;
        // 最小
        NSNumber *min =  self.xValues.firstObject;
        NSInteger minPrice = min.integerValue;
        //大小差价
        ...
        CGPoint center = CGPointMake((i+1)*self.barGap + i*self.barWidth, self.chartHeight - c/self.yAxisMaxValue * self.chartHeight + self.topMargin);
        ...
        if (self.isShowPointLabel) {
          title =  i == 0?[NSString stringWithFormat:@"最低价\n¥%@",title]:[NSString stringWithFormat:@"¥%@",title];
...
//        c = value.integerValue;
        CGSize buttonSize = CGSizeMake(self.barWidth, c/(double)self.yAxisMaxValue * self.chartHeight);
...
- (NSString *) decimalwithFormat:(NSString *)format  floatV:(float)floatV
{
    ...
    numberFormatter.minimumFractionDigits = 1;    //设置最小小数点后的位数
...
//    CGSize labelSize = [label.text sizeWithAttributes:attr];
    ...
    CGSize labelSize = tag==0?CGSizeMake(self.barWidth, 21): CGSizeMake(self.barWidth, 13);
