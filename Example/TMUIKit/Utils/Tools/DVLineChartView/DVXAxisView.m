...
- (void)drawLineInRect:(CGRect)rect withPlot:(DVPlot *)plot isPoint:(BOOL)isPoint {
    ...
    if (isPoint) {  // 画点
        ...
        for (int i = 0; i < plot.pointArray.count; i++) {
            ...
            if(i == 0 )
            {
                [button setBackgroundImage:[UIImage imageNamed:@"圆2"] forState:UIControlStateNormal];
                button.size = CGSizeMake(dot_1R, dot_1R);
                button.layer.cornerRadius = dot_1R * 0.5;
            }else
            {
                [button setBackgroundImage:[UIImage imageNamed:@"圆点"] forState:UIControlStateNormal];
...
//    label.font = self.textFont;
    ...
    if (i == 0)
    {
        ...
        button.backgroundColor = X16Color(@"#FF531A");
