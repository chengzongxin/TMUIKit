...
- (void)reset
{
    for (int x = 0; x<_tagLabels.count; x++) {
        TTGTextTagLabel *XLabel = _tagLabels[x];
        XLabel.selected = NO;
        [self reload];
    }
}
    ...
    if (_enableTagSelection) {
        ...
        if(self.isRadioSelect)
        {
            for (int x = 0; x<_tagLabels.count; x++) {
                ...
                [self updateStyleAndFrameForLabel:XLabel];
...
- (void)setIsRadioSelect:(BOOL)isRadioSelect
{
    _isRadioSelect = isRadioSelect;
}
@end
