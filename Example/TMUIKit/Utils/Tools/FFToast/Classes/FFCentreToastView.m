...
-(void)layoutToastView{
    ...
    if(_isCustomToastView == NO){
        ...
        switch (self.toastType) {
            ...
            case FFToastTypeSuccess: {
                if (!_iconImage) {
                    self.iconImage = [UIImage ff_imageWithName:@"fftoast_success_highlight"];
                }
                break;
            }
            case FFToastTypeError: {
                if (!_iconImage) {
                    self.iconImage = [UIImage ff_imageWithName:@"fftoast_error_highlight"];
                }
                break;
            }
            case FFToastTypeWarning: {
                if (!_iconImage) {
                    self.iconImage = [UIImage ff_imageWithName:@"fftoast_warning_highlight"];
                }
                break;
            }
            case FFToastTypeInfo: {
                ...
                if (!_iconImage) {
                    self.iconImage = [UIImage ff_imageWithName:@"fftoast_info_highlight"];
                }
                break;
            }
    ...
    self.frame = CGRectMake(0, 0, kMTFYScreenW, kMTFYScreenH);
...
-(CGRect)toastViewFrame{
    ...
    if(_isCustomToastView == NO){
        ...
        toastViewW = kMTFYScreenW - 2 * HORIZONTAL_SPACE_TO_SCREEN;
        toastViewX = (kMTFYScreenW - toastViewW)/2;
        
        CGFloat textMaxWidth = kMTFYScreenW - 2 * (HORIZONTAL_SPACE_TO_SCREEN + HORIZONTAL_SPACE_TO_TOASTVIEW);
        ...
        toastViewY = (kMTFYScreenH - toastViewH)/2;
...
-(void)layoutSubviews{
    ...
    if(_enableDismissBtn == YES){
        if (_dismissBtnImage != nil) {
            ...
            [self.dismissBtn setImage:[UIImage ff_imageWithName:@"fftoast_dismiss"] forState:UIControlStateNormal];
        }
