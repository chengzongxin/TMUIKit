...
- (instancetype)init {
    return [self initWithFrame:CGRectMake(0, 0, kMTFYScreenW, 50)];
}
...
-(instancetype)initWithFrame:(CGRect)frame surplusCount:(NSInteger)surplusCount {
    ...
    if(self){
        ...
        _label.text = kLStr(@"common_load_data");
                    ...
                    if(lastRow >= 0) {
                        if(tableView.visibleCells.count > 0) {
                            ...
                            if(indexPath.section == lastSection && indexPath.row == lastRow) {
                                self.frame = CGRectMake(0, CGRectGetMaxY(tableView.visibleCells.lastObject.frame), kMTFYScreenW, _originalFrame.size.height);
                            }
                        }
                    }
                }
                    ...
                    if(lastRow >= 0) {
                        if(collectionView.indexPathsForVisibleItems.count > 0) {
                            ...
                            if(indexPath.section == lastSection && indexPath.row == lastRow) {
                                ...
                                self.frame = CGRectMake(0, CGRectGetMaxY(cell.frame), kMTFYScreenW, _originalFrame.size.height);
                            }
                        }
                    }
                }
...
- (void)setLoadingType:(LoadingType)loadingType {
    ...
    switch (loadingType) {
        ...
        case LoadStateLoading:{
            ...
            [_label setText:kLStr(@"common_load_content_loading")];
        ...
        case LoadStateAll: {
            ...
            [_label setText:kLStr(@"common_load_end")];
            [_label mas_updateConstraints:^(MASConstraintMaker *make) {
        ...
        case LoadStateFailed:{
            [self setHidden:NO];
            [_indicatorView setHidden:YES];
            [_label setText:kLStr(@"common_load_more")];
            [_label mas_updateConstraints:^(MASConstraintMaker *make) {
                make.center.equalTo(self);
            }];
            [self stopAnim];
...
- (void)updateFrame {
    if([self.superView isKindOfClass:[UITableView class]]) {
        ...
        self.frame = CGRectMake(0, y, kMTFYScreenW, _originalFrame.size.height);
    }
