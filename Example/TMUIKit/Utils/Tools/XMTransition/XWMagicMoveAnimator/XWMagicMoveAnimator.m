...
#import "UIImageView+CellFrame.h"
    ...
    if (self.dampingEnable) {
        [UIView animateWithDuration:flag ? self.toDuration : self.backDuration delay:0.0f usingSpringWithDamping:0.55 initialSpringVelocity:1 / 0.55 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            [tempViewGroup enumerateObjectsUsingBlock:^(UIView *tempSubView, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                if (!flag) { // 返回才做圆角
                    tempSubView.layer.cornerRadius = endViewGroup.firstObject.layer.cornerRadius;
                    if (tempSubView.layer.cornerRadius == 0) {
                        // imageView半圆角
                        UIBezierPath *maskPath  = [UIBezierPath bezierPathWithRoundedRect:tempSubView.bounds byRoundingCorners:UIRectCornerTopLeft | UIRectCornerTopRight cornerRadii:CGSizeMake(6, 6)];
                        CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
                        maskLayer.frame         = tempSubView.bounds;
                        maskLayer.path          = maskPath.CGPath;
                        tempSubView.layer.mask      = maskLayer;
                    }
                    tempSubView.layer.masksToBounds = YES;
                }
            }];
        ...
        [UIView animateWithDuration:flag ? self.toDuration : self.backDuration animations:^{
            ...
            [tempViewGroup enumerateObjectsUsingBlock:^(UIView *tempSubView, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                if (!flag) { // 返回才做圆角
                    ...
                    if (tempSubView.layer.cornerRadius == 0) {
                        // imageView半圆角
                        UIBezierPath *maskPath  = [UIBezierPath bezierPathWithRoundedRect:tempSubView.bounds byRoundingCorners:UIRectCornerTopLeft | UIRectCornerTopRight cornerRadii:CGSizeMake(12, 12)];
                        CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
                        maskLayer.frame         = tempSubView.bounds;
                        maskLayer.path          = maskPath.CGPath;
                        tempSubView.layer.mask      = maskLayer;
...
- (NSArray *)_xw_getFrameValueFromViewGroup:(NSArray<UIView *> *)group inContainerView:(UIView *)containerView{
    ...
    [group enumerateObjectsUsingBlock:^(UIView *view, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([view isKindOfClass:[UIImageView class]]) {
            ...
            if (imageV.cellFrame.size.width && imageV.cellFrame.size.height) {
                [temp addObject:[NSValue valueWithCGRect:imageV.cellFrame]];
            }else{
