    ...
    dispatch_source_set_event_handler(timer, ^{
        if (tempSecond <= 1) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                self.enabled = YES;
                [self setTitle:kLStr(@"login_index_getVerifyCode") forState:UIControlStateNormal];
            });
        } else {
            tempSecond--;
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)SG_countdownWithSecond:(NSInteger)second {
    ...
    dispatch_source_set_event_handler(timer, ^{
        if (tempSecond <= 1) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                self.enabled = NO;
                [self setTitle:[NSString stringWithFormat:@"%ld%@", (long)tempSecond, kLStr(@"common_time_second")] forState:UIControlStateNormal];
            });
        }
    });
    dispatch_resume(timer);
