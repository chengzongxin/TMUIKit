...
@interface TMShowImageAnimatorTransition ()
...
@implementation TMShowImageAnimatorTransition
...
//    NSLog(@" percent = %f, scale = %f",percent,scale);
    ...
    switch (panGesture.state) {
        ...
        case UIGestureRecognizerStateEnded:{
            ...
            void (^animateFinishBlock)(BOOL) = ^(BOOL finished) {
                self.animateImageView.transform = CGAffineTransformIdentity;
                self.animateImageView.hidden = YES;
                self.originImageView.hidden = NO;
            };
            
            if (self.originImageView.layer.cornerRadius) {
                [UIView animateWithDuration:0.5 animations:animateBlock completion:animateFinishBlock];
            }else{
                [UIView animateWithDuration:0.5 delay:0.0 usingSpringWithDamping:0.8 initialSpringVelocity:0.7 options:UIViewAnimationOptionCurveLinear animations:animateBlock completion:animateFinishBlock];
            }
...
- (void)popAnimation:(id<UIViewControllerContextTransitioning>)transitionContext{
    TMShowBigImageViewController *fromVC = [transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    UIViewController *toVC = [transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    UIView *containerView = [transitionContext containerView];
